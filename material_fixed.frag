...
{
	local = co;
	view = (my_ProjectionMatrix[3][3] == 0.0)? normalize(local): vec3(0.0, 0.0, -1.0);
...
{
	mat3 obj2view = to_mat3(my_ModelViewMatrix);
	mat3 view2obj = to_mat3(my_ModelViewMatrixInverse);
...
{
	outnormal = normalize(my_NormalMatrix * texnormal);
}
...
void shade_view(vec3 co, out vec3 view)
{
	...
	view = (my_ProjectionMatrix[3][3] == 0.0)? normalize(co): vec3(0.0, 0.0, -1.0);
}
...
void shade_mist_factor(vec3 co, float enable, float miststa, float mistdist, float misttype, float misi, out float outfac)
{
	if(enable == 1.0) {
		...
		zcor = (my_ProjectionMatrix[3][3] == 0.0)? length(co): -co[2];
...
{
	vec4 v = (my_ProjectionMatrix[3][3] == 0.0) ? vec4(viewvec, 1.0) : vec4(0.0, 0.0, 1.0, 1.0);
	vec4 co_homogenous = (my_ProjectionMatrixInverse * v);

	vec4 co = vec4(co_homogenous.xyz / co_homogenous.w, 0.0);
	worldvec = (my_ModelViewMatrixInverse * co).xyz;
}
...
{
	/* handle perspective/orthographic */
	vec3 I_view = (my_ProjectionMatrix[3][3] == 0.0)? normalize(I): vec3(0.0, 0.0, -1.0);
	...
	out vec3 camera, out vec3 window, out vec3 reflection)
{
	...
	vec4 projvec = my_ProjectionMatrix * vec4(I, 1.0);
...
void node_tex_coord_background(vec3 I, vec3 N, mat4 viewinvmat, mat4 obinvmat, vec4 camerafac,
	...
	out vec3 camera, out vec3 window, out vec3 reflection)
{
	vec4 v = (my_ProjectionMatrix[3][3] == 0.0) ? vec4(I, 1.0) : vec4(0.0, 0.0, 1.0, 1.0);
	vec4 co_homogenous = (my_ProjectionMatrixInverse * v);

	vec4 co = vec4(co_homogenous.xyz / co_homogenous.w, 0.0);

	co = normalize(co);
	vec3 coords = (my_ModelViewMatrixInverse * co).xyz;
	...
	window = (my_ProjectionMatrix[3][3] == 0.0) ? 
...
const vec4 cons46 = vec4(0.000000, 0.000000, 0.000000, 0.005000);
uniform float unf57;
uniform float unf60;
uniform vec3 unf66;
const float cons76 = float(0.171191);
uniform vec4 unf77;
...
vec4 heatmap(float value, float minValue, float maxValue)
{
    ...
    float ratio=(HEATMAP_COLORS_COUNT-1.0)*clamp((value-minValue)/(maxValue-minValue),0,1);
...
void main(void)
{
	...
	lamp_visibility_spot_circle(unf19, tmp13, tmp21);
	lamp_visibility_spot(unf22, unf23, tmp21, tmp18, tmp26);
	lamp_visibility_clamp(tmp26, tmp28);
	shade_inp(tmp4, tmp13, tmp31);
	shade_visifac(tmp31, tmp28, unf34, tmp35);
	shade_mul_value_v3(unf36, unf37, tmp38);
	shade_mul_value(tmp35, vec4(tmp38, 1.0), tmp41);
	mtex_value_invert(cons42, tmp43);
	mix_mult(tmp43, tmp41, cons46, tmp47);
	mtex_value_invert(tmp43, tmp49);
	shade_madd(tmp9, tmp47, vec4(tmp2, 1.0), tmp53);
	shade_cooktorr_spec(tmp4, tmp13, tmp6, unf57, tmp58);
	shade_spec_t(tmp49, unf60, tmp28, tmp58, tmp63);
	shade_add_spec(tmp63, tmp38, unf66, tmp67);
	shade_add_clamped(vec4(tmp10, 1.0), vec4(tmp67, 1.0), tmp70);
	shade_clamp_positive(tmp70, tmp72);
	shade_clamp_positive(tmp53, tmp74);
	shade_maddf(tmp74, cons76, unf77, tmp78);
	shade_add(tmp78, tmp72, tmp81);
	mtex_alpha_to_col(tmp81, cons83, tmp84);
	shade_mist_factor(varposition, unf86, unf87, unf88, unf89, unf90, tmp91);
	mix_blend(tmp91, tmp84, unf94, tmp95);
	shade_alpha_opaque(tmp95, tmp97);
	linearrgb_to_srgb(tmp97, tmp99);

    // gl_FragColor = vec4(tmp18*10,0,0,1); // falloff
    // gl_FragColor = vec4(tmp26*10,0,0,1); // final spot
    // gl_FragColor = vec4(normalize(varposition - unf12),1); // pos -> lamp vector
    
    // float z = length(varposition - unf12) / 200; //tmp26;
	// gl_FragColor = vec4(z,z,z,1); // pos -> lamp pos length
    
    /* non-orthoNormalize fix:
    vec3 varposition_fixed = vec3(my_ModelViewMatrixInverse * vec4(varposition,1));
    vec3 test_vec = vec3(0,0,-1);
    vec3 test_pos = vec3(10,0,32);
    vec3 v1 = my_NormalMatrix * normalize(varposition_fixed - test_pos);
    vec3 v2 = my_NormalMatrix * test_vec;
    gl_FragColor = heatmap(dot(v1,v2), 0, M_PI);
    */

    /* only works when orthonormal:
    vec3 varposition_fixed = vec3(my_ModelViewMatrixInverse * vec4(varposition,1));
    vec3 test_vec = vec3(0,0,-1);
    vec3 test_pos = vec3(10,0,32);
    vec3 v1; // = vec3(my_ModelViewMatrix * vec4(normalize(varposition_fixed - test_pos), 0));
    v1 = normalize(varposition - vec3(my_ModelViewMatrix * vec4(test_pos,1)));
    vec3 v2 = vec3(my_ModelViewMatrix * vec4(test_vec,0));
    gl_FragColor = heatmap(dot(v1,v2), 0, M_PI);
    */
    ...
    vec3 test_vec = unf19;
    vec3 test_pos = unf12;
    vec3 v1 = normalize(varposition - test_pos);
    vec3 v2 = test_vec;
    gl_FragColor = heatmap(dot(v1,v2), 0, M_PI);
